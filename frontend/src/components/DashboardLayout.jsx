        ...
        <div className={`flex h-screen overflow-hidden ${bgClass} font-sans transition-colors duration-300 relative`}>
            ...
            {role === 'student' ? (
                <div 
                    ...
                    style={{ 
                        backgroundImage: "url('https://images.unsplash.com/photo-1576091160399-11cbbe989951?q=80&w=2000&auto=format&fit=crop')", 
            ...
            <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out ${sidebarBg} flex flex-col`}>
                <div className="p-6 flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-black tracking-tight" style={{ color: isDarkUI ? '#fff' : '#0f172a' }}>Med-Zukkoo</h2>
                ...
                <div className="p-4 border-t border-slate-700/30">
                    <button 
                        ...
                        ${isDarkUI ? 'bg-rose-500/10 hover:bg-rose-500/20 text-rose-400' : 'bg-rose-50 border border-rose-100 hover:bg-rose-100 text-rose-600'}`}
                    >
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden relative z-10 min-w-0">
                ...
                <header className={`h-16 flex items-center justify-between px-6 backdrop-blur-md ${headerBg} sticky top-0 z-30`}>
                    ...
                    <div className="flex items-center gap-2 sm:gap-4 relative">
                        {/* Language switcher */}
                        <div className={`hidden sm:flex items-center rounded-xl overflow-hidden border ${isDarkUI ? 'border-slate-700 bg-slate-900' : 'border-slate-200 bg-slate-100'}`}>
                            {['uz', 'ru', 'en'].map(l => (
                                <button
                                    key={l}
                                    onClick={() => toggleLang(l)}
                                    className={`px-2 sm:px-3 py-1.5 text-[10px] font-bold uppercase transition-colors ${lang === l ? 'bg-blue-600 text-white' : (isDarkUI ? 'text-slate-400 hover:bg-slate-800' : 'text-slate-500 hover:bg-slate-200')}`}
                                >
                                    {l}
                                </button>
                            ))}
                        ...
                        <button
                            onClick={toggleTheme}
                            className={`w-9 h-9 rounded-full flex items-center justify-center transition-colors ${isDarkUI ? 'bg-slate-800 hover:bg-slate-700 text-yellow-500 border border-slate-700' : 'bg-slate-100 hover:bg-slate-200 text-blue-600 border border-slate-200'}`}
                        >
                        ...
                        <div className="relative">
                            <button onClick={toggleNotifications} className={`p-2 w-9 h-9 flex items-center justify-center rounded-full relative cursor-pointer hidden sm:flex ${isDarkUI ? 'bg-slate-800 hover:bg-slate-700 border border-slate-700' : 'bg-slate-100 hover:bg-slate-200 border border-slate-200'}`}>
                                        ...
                                        <div className="max-h-64 overflow-y-auto">
                                            {notifications.length === 0 ? (
                                                ...
                                                notifications.map((n, i) => (
                                                    <div key={n.id} className={`p-4 ${i !== notifications.length - 1 ? 'border-b' : ''} flex gap-3 hover:bg-slate-500/5 transition-colors cursor-pointer ${isDarkUI ? 'border-slate-800 text-slate-300' : 'border-slate-100 text-slate-600'}`}>
                                                        ...
                                                        <div>
                                                            <p className={`text-xs font-bold leading-tight ${isDarkUI ? 'text-slate-200' : 'text-slate-800'}`}>{n.title}</p>
                        ...
                        >
                            ...
                            <div className="hidden md:block text-sm">
                                <p className="font-bold" style={{ color: isDarkUI ? '#fff' : '#0f172a' }}>{user?.displayName?.split(' ')[0]}</p>
                            </div>
                        </div>
                    </div>
                </header>
